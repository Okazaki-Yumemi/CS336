...
从代码来梳理，我们的目的就是拿到之前的梯度，做反向的梯度下降。
...
```
主要是准备各种数据，需要的raw_rewards，advantages,和tokenized里面的input_ids,labels,response_mask。
...
前面都是在切片，然后用log_probs得到对数概率，传入算gradientloss的函数里面取得到逐token的loss，然后再aggregate成microbatch的loss，最后scaled_loss.backward()做反向传播。

函数外，通过梯度裁剪和optimizer.step()和optimizer.zero_grad()来更新参数。
